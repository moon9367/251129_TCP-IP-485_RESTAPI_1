...
class ModbusController:
    ...
    def __init__(self, host="168.131.153.52", port=9139, unit_id=1, timeout=5, retries=3):
        """
        초기화
    ...
    def connect(self):
        ...
        try:
            ...
            if result:
                ...
                logger.error(f"연결 실패: {self.host}:{self.port}")
                return False
    ...
    def read_sensor(self, address, scale=1):
        """
        센서값 읽기 (스케일 적용)
        ...
        logger.info(f"센서 읽기: 주소 {address}, Raw={raw_value}, 실제값={actual_value}")
        return actual_value
    ...
    def write_sensor_value(self, address, value, scale=1):
        """
        센서 설정값 쓰기 (스케일 적용)
        ...
        logger.info(f"센서 설정값 쓰기: 주소 {address}, 실제값={value}, 레지스터값={register_value}")
    ...
    def write_by_name(self, name, value):
        ...
        try:
            if spec_type == 'REGISTER_WRITE':
                ...
                logger.info(f"[{name}] 쓰기: {value} → {register_value}")
                return result
